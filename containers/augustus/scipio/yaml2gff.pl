...
#
#
#   This script transforms the yaml output of BlatWrapper into easily readable GFF 
#   
...
#
#   the protein sequence shown contains the following extra symbols:
...
sub output {
	...
	if (defined $prev_end && $prev_end < $querystart) {
...
print "##gff-version\t3\n";
print STDERR "Found ".(scalar @input)." YAML entries.\n";
foreach (@input) {
    s/^/---/;
    s/$/\n/;
    my $hits = YAML::Load($_);
