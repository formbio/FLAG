process trinity {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/trinity.sh -f ${bam} -s ${filetype} -n ${nameType} -t 95 -m 320G -i 10000
  """
  stub:
  """
...
process exonerate_p2g {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/exonerate_p2g.sh -g ${genome} -p ${protein} -n 6270 -t 95
  """
  stub:
  """
  touch trinity_out_dir.Trinity.fasta
...
process prosplign {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/tblastn_prosplign.sh -g ${genome} -p ${protein} -n 1 -t 95
  """
  stub:
  """
  touch trinity_out_dir.Trinity.fasta
...
process pasa {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/pasa.sh -g ${genome} -r ${transcript} -t 16
  """
  stub:
  """
  touch trinity_out_dir.Trinity.fasta
...
process genome2protein {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/genome2protein.sh -i ${genome} -d ${database} -a diamond -t 31
  """
  stub:
  """
...
process genome2transcriptome {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/genome2transcriptome.sh -i ${genome} -d ${database} -t 46
  """
  stub:
  """
...
process trnascan {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/tRNAScan.sh -g ${genome} -t 46
  """
  stub:
  """
  touch tRNAScan.gff3
...
process augustus {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/augustus_pasa.sh -g ${genome} -r ${transcript} -s ${species} -n 31 -e ${exoneratehints} -p ${pretrained}
  """
  stub:
  """
  touch augustus.gtf
...
process otherToolTrainAugustus {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/augustus_pasa.sh -g ${genome} -r ${transcript} -s ${species} -n 31 -p False
  """
  stub:
  """
  touch augustus.gtf
...
process genomethreader {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/genomethreader.sh -g ${genome} -p ${protein} -t 95
  """
  stub:
  """
  touch trinity_out_dir.Trinity.fasta
...
process miniprot {
  ...
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/miniprot.sh -g ${genome} -p ${protein} -t 95
  """
  stub:
  """
  touch trinity_out_dir.Trinity.fasta
...
process combinestructwfunct {
  ...
  stageOutMode = 'copy'
  input:
    tuple path(entap),path(annotation),val(species)
  output:
    path("final*.gtf")
  script:
  """
  bash ${params.repoDir}/scripts/geneannotation/parseEntap.sh -i ${entap} -a ${annotation} -s ${species}
  """
  stub:
  """
  touch final_annotations_lvl0.tsv
