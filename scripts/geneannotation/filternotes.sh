...
if [ "${input}" == "reference_augustus.gtf" ]; then
    mv filteredAnnots.gtf filtered_reference_augustus.gtf
    mv removedAnnots.gtf removed_reference_augustus.gtf
elif [ "${input}" == "augustus.gff" ]; then
    mv filteredAnnots.gtf filtered_augustus.gtf
    mv removedAnnots.gtf removed_augustus.gtf
fi

#for liftoff filtering
grep -v '^#' full_table.tsv > full_tableEdited1.tsv
grep -v 'Missing' full_tableEdited1.tsv > full_tableEdited2.tsv
if [[ "$onlycomplete" == "True" ]]; then
    mv full_tableEdited2.tsv tmpfull_tableEdited2.tsv
...
while IFS= read line; do
    currentBusco="${line:1}"
    echo "$currentBusco"
    mRNALine=`grep "${currentBusco}" liftoff.gff3 | grep -P "Liftoff\tmRNA"`
    first=`echo "$mRNALine" | cut -d$'\t' -f 4`
    second=`echo "$mRNALine" | cut -d$'\t' -f 5`
    #geneLine="#duplicate"
    geneLine=`grep -P "Liftoff\tgene\t${first}\t${second}" liftoff.gff3`
    echo "$geneLine"
    if [[ "$geneLine" != "" ]]; then
       echo "$geneLine" >> testfile.gff3
       grep -v -P "Liftoff\tgene\t${first}\t${second}" liftoff.gff3 > tmpfile && mv tmpfile liftoff.gff3
    fi
    grep "=${currentBusco};" liftoff.gff3 >> testfile.gff3
    grep -v "=${currentBusco};" liftoff.gff3 > tmpfile && mv tmpfile liftoff.gff3
done < "full_tableEdited3Sorted.tsv"



#for helixer filtering
grep -v '^#' full_table.tsv > full_tableEdited1.tsv
grep -v 'Missing' full_tableEdited1.tsv > full_tableEdited2.tsv
if [[ "$onlycomplete" == "True" ]]; then
    mv full_tableEdited2.tsv tmpfull_tableEdited2.tsv
...
while IFS= read line; do
        currentBusco=`echo "${line}" | awk '{ print $3 }' | awk -Fg '{print $NF}'`
        echo "${currentBusco::-2}" >> full_tableEdited3.tsv
done < "full_tableEdited2.tsv"
...
while IFS= read line; do
    ...
    #grep "${currentBusco}" annot.gff3 >> testfile.gff3
    #grep -v -P "${currentBusco}" annot.gff3 > tmpfile && mv tmpfile annot.gff3
    grep "ID=${currentBusco::-2}$" annot.gff3 >> testfile.gff3
    grep "Parent=${currentBusco::-2}$" annot.gff3 >> testfile.gff3
    grep "Parent=${currentBusco}$" annot.gff3 >> testfile.gff3
    grep -v "ID=${currentBusco::-2}$" annot.gff3 > tmpfile && mv tmpfile annot.gff3
    grep -v "Parent=${currentBusco::-2}$" annot.gff3 > tmpfile && mv tmpfile annot.gff3
    grep -v "Parent=${currentBusco}$" annot.gff3 > tmpfile && mv tmpfile annot.gff3
done < "full_tableEdited3Sorted.tsv"
...
while IFS= read line; do
    ...
    echo "$currentBusco"
    echo "yes"
    grep "ID=evm.TU${currentBusco:9};" annot.gff3 >> testfile.gff3
    testvar=`grep "ID=evm.TU${currentBusco:9};" annot.gff3`
    echo "$testvar"
    echo `grep "Parent=evm.TU${currentBusco:9};" annot.gff3`
    grep "Parent=evm.TU${currentBusco:9};" annot.gff3 >> testfile.gff3
    grep "Parent=${currentBusco}$" annot.gff3 >> testfile.gff3
    grep -v "ID=evm.TU${currentBusco:9};" annot.gff3 > tmpfile && mv tmpfile annot.gff3
    grep -v "Parent=evm.TU${currentBusco:9};" annot.gff3 > tmpfile && mv tmpfile annot.gff3
    grep -v "Parent=${currentBusco}$" annot.gff3 > tmpfile && mv tmpfile annot.gff3
done < "full_tableEdited3Sorted.tsv"
