...
usage() {
  ...
  echo "-m  --max RAM"
  echo "-i  --max intron"
  echo "-p  --preprocess. If true run trimmomatic. "
  echo "Example: bash trinity.sh -f LIB034841_TRA00126514_S11_L001_R1.fastq -r LIB034841_TRA00126514_S11_L001_R2.fastq -t 30 -m 120G -p true"
  exit 1
}

OPTIND=1 # Reset OPTIND
while getopts :f:r:t:m:n:s:i:p:h opt
do
    case $opt in
        f) f1=$OPTARG;;
        r) f2=$OPTARG;;
...
if [ "$seqtype" == "bam" ]; then
    ...
    then
        ...
        Trinity $opt --seqType $seqtype --single $f1 --CPU $cpu --max_memory $mem --bflyHeapSpaceMax "180G"
        ...
        Trinity $opt --seqType $seqtype --left $f1 --right $f2 --CPU $cpu --max_memory $mem --bflyHeapSpaceMax "80G"
    fi
fi
